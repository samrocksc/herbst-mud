...
type QuestQuery struct {
	...
	withProgress *QuestProgressQuery
...
func (_q *QuestQuery) QueryProgress() *QuestProgressQuery {
...
func (_q *QuestQuery) WithProgress(opts ...func(*QuestProgressQuery)) *QuestQuery {
	query := (&QuestProgressClient{config: _q.config}).Query()
...
func (_q *QuestQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Quest, error) {
	...
	if query := _q.withProgress; query != nil {
		if err := _q.loadProgress(ctx, query, nodes,
			func(n *Quest) { n.Edges.Progress = []*QuestProgress{} },
			func(n *Quest, e *QuestProgress) { n.Edges.Progress = append(n.Edges.Progress, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *QuestQuery) loadProgress(ctx context.Context, query *QuestProgressQuery, nodes []*Quest, init func(*Quest), assign func(*Quest, *QuestProgress)) error {
	...
	query.Where(predicate.QuestProgress(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(quest.ProgressColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.quest_progress
		if fk == nil {
			return fmt.Errorf(`foreign-key "quest_progress" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "quest_progress" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
