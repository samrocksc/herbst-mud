...
func (_q *UserQuery) loadCharacters(ctx context.Context, query *CharacterQuery, nodes []*User, init func(*User), assign func(*User, *Character)) error {
	...
	for _, n := range neighbors {
		fk := n.user_characters
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_characters" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_characters" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
