...
async function apiFetch<T = unknown>(input: RequestInfo, init?: RequestInit): Promise<T> {
  ...
  try {
    ...
    if (keys.length <= 2) {
      const unwrapKeys = ["skills", "npcs", "characters", "abilities", "users", "items", "rooms", "races", "achievements", "factions", "faction_categories", "effects", "hooks", "active_effects", "tags", "recipes", "quests", "quest_progress", "dialog_nodes", "genders", "zones", "event_logs"];
