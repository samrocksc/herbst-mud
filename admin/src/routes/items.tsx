...
function ItemManager() {
  ...
  if (loading) {
    return <div className="p-8 text-white">Loading items...</div>
  }
    ...
    <div className="flex h-screen bg-[#0a0a0f]">
      ...
      <div className="w-[280px] bg-[#1a1a2e] border-r border-[#333] flex flex-col">
        <div className="p-4 border-b border-[#333]">
          ...
          <Link
            to="/map"
            className="block text-[#888] no-underline p-2 rounded bg-[#16213e] text-center mb-2 hover:bg-[#1e2a4a]"
          >
            Map Builder
          </Link>
          <Link
        ...
        <div className="p-3 border-b border-[#333]">
          <h2 className="m-0 text-white text-lg">Item Manager</h2>
          <p className="text-[#666] text-xs mt-1 mb-0">{items.length} items</p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-3">
          <div className="flex flex-col gap-1">
            {filteredItems.map(item => (
              <div
                ...
                className={`p-2 cursor-pointer rounded text-xs ${
                  selectedItem?.id === item.id ? 'text-[#3498db] bg-[#16213e]' : 'text-white'
                }`}
              >
            ...
            {filteredItems.length === 0 && (
              <div className="text-[#666] text-center py-4">No items found</div>
            )}
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {showCreateForm ? (
          <div className="max-w-[600px] mx-auto">
            <h2 className="mt-0 mb-4 text-white">Create New Item</h2>
            ...
            <div className="bg-[#1a1a2e] rounded-lg p-4">
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  <label className="text-[#888] text-xs block mb-1">Slot</label>
                  <select
                    value={form.slot}
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  <label className="text-[#888] text-xs block mb-1">Weight</label>
                  <input
                    type="number"
                    value={form.weight}
          ...
          <div className="max-w-[600px] mx-auto">
            <h2 className="mt-0 mb-4 text-white">Edit Item</h2>
          ...
          <div className="max-w-[600px] mx-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="m-0 text-white">{selectedItem.name}</h2>
              <button
            ...
            <div className="bg-[#1a1a2e] rounded-lg p-4">
              <div className="mb-4">
                <div className="text-[#666] text-xs mb-1">ID: #{selectedItem.id}</div>
                <div className="text-white">{selectedItem.description || 'No description'}</div>
              </div>

              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  <label className="text-[#888] text-xs block mb-1">Type</label>
                  <div className="text-white">{selectedItem.itemType}</div>
                </div>
                <div>
                  <label className="text-[#888] text-xs block mb-1">Slot</label>
                  <div className="text-white">{selectedItem.slot || 'none'}</div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  <label className="text-[#888] text-xs block mb-1">Level</label>
                  <div className="text-white">{selectedItem.level}</div>
                </div>
                <div>
                  <label className="text-[#888] text-xs block mb-1">Weight</label>
                  <div className="text-white">{selectedItem.weight}</div>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  <label className="text-[#888] text-xs block mb-1">Visible</label>
                  <div className="text-white">{selectedItem.isVisible ? 'Yes' : 'No'}</div>
                </div>
                <div>
                  <label className="text-[#888] text-xs block mb-1">Immovable</label>
                  <div className="text-white">{selectedItem.isImmovable ? 'Yes' : 'No'}</div>
                </div>
              </div>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 p-2 bg-[#3498db] border-none rounded text-white cursor-pointer hover:bg-[#2980b9]"
                >
                  Edit Item
                </button>
                <button
