...
export const Route = createFileRoute("/_auth/characters/$characterId/examine")({
  component: CharacterExaminePage,
});
...
type CharacterExamineResponse = Readonly<{
  name: string;
  description: string;
  race: string;
  level: number;
...
function CharacterExaminePage() {
  ...
  // Load character data
  const { data: character, isLoading: charLoading, isError: charError, error: charErrorObj } = useCharacter(id);
  ...
  const { data: examineData, isLoading: examineLoading, isError: examineError, error: examineErrorObj } =
    useCharacterExamine(id, examineConfig);

  // Combined loading/error state
  const isLoading = charLoading || examineLoading;
  const isError = charError || examineError;
  const error = charErrorObj?.message || examineErrorObj?.message;

  if (isLoading) return <LoadingState />;
  if (isError) return <ErrorState error={error} />;

  // Merge character data with examine data
  const name = character?.name || examineData?.name || "Unknown";
  const race = character?.race || examineData?.race || "Unknown";
  const level = character?.level ?? examineData?.level ?? 0;
  const description = character?.description || examineData?.description || "";
  const equippedItems = examineData?.equipped_items ?? [];
  const unequippedItems = examineData?.unequipped_items ?? [];

  return (
    <div className="p-6 max-w-[800px] mx-auto">
      <PageHeader title={name} showBack backTo={`/characters/${id}`} />
...
function useCharacterExamine(
  ...
  return useQuery({
    queryKey: ["character-examine", characterId, params],
    queryFn: async (): Promise<CharacterExamineResponse | null> => {
      const data = await apiGet<CharacterExamineResponse>(
        `${window.location.origin}/api/characters/${characterId}/examine${params ? `?${params}` : ""}`
      );
      return data ?? null;
    },
  });
...
function LoadingState() {
  return (
    <div className="p-8">
      <PageHeader title="Loading..." showBack backTo="/characters" />
      <div className="text-text-muted">Loading character details...</div>
    </div>
  );
}
...
function ErrorState({ error }: { error: string | undefined }) {
  return (
    <div className="p-8">
      <PageHeader title="Error" showBack backTo="/characters" />
