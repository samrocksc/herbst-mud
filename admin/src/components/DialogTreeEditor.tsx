...
interface DialogTreeEditorProps {
  ...
  readOnly?: boolean;
  onChange?: (nodes: DialogNodeInput[]) => void;
}
...
export function DialogTreeEditor({ npcTemplateId, readOnly = false, onChange }: DialogTreeEditorProps) {
  ...
  const notifyChange = (updatedNodes: DialogNodeInput[]) => {
    onChange?.(updatedNodes);
  };

  const handleAdd = async () => {
    const id = newId || `node_${Date.now()}`;
    const input: DialogNodeInput & { npc_template_id: string } = {
      id,
      npc_text: newText || "...",
    ...
    if (onChange) {
      ...
      notifyChange([...currentNodes, input]);
    }
  ...
  return (
    <div className="mt-4">
      <div className="flex items-center justify-between mb-4">
        <h3 className="m-0 text-text text-base font-semibold">Dialog Tree</h3>
        {!readOnly && (
      ...
      {adding && !readOnly && (
        <div className="form-card mb-4 space-y-3">
          ...
          <div className="flex items-center gap-2">
            <input type="checkbox" checked={newEntry} onChange={(e) => setNewEntry(e.target.checked)} id="new-entry-editor" disabled={!!entryNode} />
            <label htmlFor="new-entry-editor" className="text-sm text-text">Entry Node</label>
          </div>
      ...
      {sorted.length === 0 && !adding && (
        <div className="text-text-muted text-sm py-4">No dialog nodes yet.</div>
      )}

      {sorted.map((node) => (
        <div key={node.id} className="form-card mb-3">
          {editing === node.id && editForm && !readOnly ? (
      ...
      {!readOnly && (
        <DeleteConfirmation
          ...
          message="Are you sure? This will permanently delete this dialog node."
