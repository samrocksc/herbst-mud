    ...
    <Modal isOpen={showSpawn} onClose={() => setShowSpawn(false)} title="Spawn NPC Instance">
      <div className="space-y-3">
        ...
        <div>
          ...
          {templatesLoading ? (
            ...
            <SearchableSelect
              ...
              onChange={(id) => setSpawnForm((f) => ({ ...f, template_id: id }))}
        ...
        <div className="flex gap-2">
          <NumberField
            label="Level (0 = default)"
            value={spawnForm.level}
            onChange={(v) => setSpawnForm((f) => ({ ...f, level: v }))}
            min={0}
            disabled={isPending}
          />
          <NumberField
            ...
            onChange={(v) => setSpawnForm((f) => ({ ...f, hitpoints: v }))}
            min={0}
            disabled={isPending}
          />
        </div>
        ...
        <NumberField
          ...
          onChange={(v) => setSpawnForm((f) => ({ ...f, room_id: v }))}
          disabled={isPending}
        />

        <div className="flex gap-2">
          <NumberField
            label="Respawn Cooldown (0 = template default)"
            value={spawnForm.respawn_cooldown}
            onChange={(v) => setSpawnForm((f) => ({ ...f, respawn_cooldown: v }))}
            min={0}
            disabled={isPending}
          />
          <FormField
            label="Respawn Rooms (empty = template default)"
            value={spawnForm.respawn_rooms}
            onChange={(v) => setSpawnForm((f) => ({ ...f, respawn_rooms: v }))}
