...
func (_q *CharacterQuery) QueryRoom() *RoomQuery {
...
// QueryNpcTemplate chains the current query on the "npcTemplate" edge.
func (_q *CharacterQuery) QueryNpcTemplate() *NPCTemplateQuery {
...
func (_q *CharacterQuery) QueryAbilities() *CharacterAbilityQuery {
...
func (_q *CharacterQuery) QueryCompetencies() *CharacterCompetencyQuery {
...
func (_q *CharacterQuery) WithRoom(opts ...func(*RoomQuery)) *CharacterQuery {
	query := (&RoomClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// the "npcTemplate" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *CharacterQuery) WithNpcTemplate(opts ...func(*NPCTemplateQuery)) *CharacterQuery {
	query := (&NPCTemplateClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *CharacterQuery) WithAbilities(opts ...func(*CharacterAbilityQuery)) *CharacterQuery {
	query := (&CharacterAbilityClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *CharacterQuery) WithCompetencies(opts ...func(*CharacterCompetencyQuery)) *CharacterQuery {
	query := (&CharacterCompetencyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *CharacterQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Character, error) {
	...
	if query := _q.withRoom; query != nil {
		if err := _q.loadRoom(ctx, query, nodes, nil,
			func(n *Character, e *Room) { n.Edges.Room = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withAbilities; query != nil {
		if err := _q.loadAbilities(ctx, query, nodes,
			func(n *Character) { n.Edges.Abilities = []*CharacterAbility{} },
			func(n *Character, e *CharacterAbility) { n.Edges.Abilities = append(n.Edges.Abilities, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *CharacterQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Character, init func(*Character), assign func(*Character, *User)) error {
	...
	for i := range nodes {
		if nodes[i].user_characters == nil {
			continue
		}
		fk := *nodes[i].user_characters
...
func (_q *CharacterQuery) loadRoom(ctx context.Context, query *RoomQuery, nodes []*Character, init func(*Character), assign func(*Character, *Room)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Character)
	for i := range nodes {
		fk := nodes[i].CurrentRoomId
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "currentRoomId" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *CharacterQuery) loadNpcTemplate(ctx context.Context, query *NPCTemplateQuery, nodes []*Character, init func(*Character), assign func(*Character, *NPCTemplate)) error {
...
func (_q *CharacterQuery) loadAbilities(ctx context.Context, query *CharacterAbilityQuery, nodes []*Character, init func(*Character), assign func(*Character, *CharacterAbility)) error {
	...
	query.Where(predicate.CharacterAbility(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(character.AbilitiesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.character_abilities
		if fk == nil {
			return fmt.Errorf(`foreign-key "character_abilities" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "character_abilities" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *CharacterQuery) loadCompetencies(ctx context.Context, query *CharacterCompetencyQuery, nodes []*Character, init func(*Character), assign func(*Character, *CharacterCompetency)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Character)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.CharacterCompetency(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(character.CompetenciesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.character_competencies
		if fk == nil {
			return fmt.Errorf(`foreign-key "character_competencies" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
