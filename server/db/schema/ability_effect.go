...
// Each ability can have multiple effects linked via the "effects" edge.
...
func (AbilityEffect) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Float("scaling_ratio").
			Default(0).
			Comment("Multiplier per point of scaling stat (e.g., 0.5 = +50% per point)"),
		field.Int("sort_order").
			Default(0).
			Comment("Order of effects within an ability (0 = first)"),
	}
}
