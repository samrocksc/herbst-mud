...
func (_q *SkillQuery) QueryWorld() *WorldQuery {
...
func (_q *SkillQuery) QueryCharacterSkills() *CharacterSkillQuery {
...
func (_q *SkillQuery) QueryAbilities() *AbilityQuery {
...
func (_q *SkillQuery) WithWorld(opts ...func(*WorldQuery)) *SkillQuery {
	query := (&WorldClient{config: _q.config}).Query()
...
func (_q *SkillQuery) WithCharacterSkills(opts ...func(*CharacterSkillQuery)) *SkillQuery {
	query := (&CharacterSkillClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *SkillQuery) WithAbilities(opts ...func(*AbilityQuery)) *SkillQuery {
	query := (&AbilityClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *SkillQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Skill, error) {
	...
	if query := _q.withWorld; query != nil {
		if err := _q.loadWorld(ctx, query, nodes, nil,
			func(n *Skill, e *World) { n.Edges.World = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withAbilities; query != nil {
		if err := _q.loadAbilities(ctx, query, nodes,
			func(n *Skill) { n.Edges.Abilities = []*Ability{} },
			func(n *Skill, e *Ability) { n.Edges.Abilities = append(n.Edges.Abilities, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SkillQuery) loadWorld(ctx context.Context, query *WorldQuery, nodes []*Skill, init func(*Skill), assign func(*Skill, *World)) error {
...
func (_q *SkillQuery) loadCharacterSkills(ctx context.Context, query *CharacterSkillQuery, nodes []*Skill, init func(*Skill), assign func(*Skill, *CharacterSkill)) error {
...
func (_q *SkillQuery) loadParent(ctx context.Context, query *SkillQuery, nodes []*Skill, init func(*Skill), assign func(*Skill, *Skill)) error {
	...
	nodeids := make(map[int][]*Skill)
	for i := range nodes {
		if nodes[i].ParentSkillID == nil {
			continue
		}
		fk := *nodes[i].ParentSkillID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "parent_skill_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *SkillQuery) loadChildren(ctx context.Context, query *SkillQuery, nodes []*Skill, init func(*Skill), assign func(*Skill, *Skill)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(skill.FieldParentSkillID)
	}
	...
	for _, n := range neighbors {
		fk := n.ParentSkillID
		if fk == nil {
			return fmt.Errorf(`foreign-key "parent_skill_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "parent_skill_id" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *SkillQuery) loadAbilities(ctx context.Context, query *AbilityQuery, nodes []*Skill, init func(*Skill), assign func(*Skill, *Ability)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Skill)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(ability.FieldRequiredSkillID)
	}
	query.Where(predicate.Ability(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(skill.AbilitiesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.RequiredSkillID
		if fk == nil {
			return fmt.Errorf(`foreign-key "required_skill_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "required_skill_id" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *SkillQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withWorld != nil {
			_spec.Node.AddColumnOnce(skill.FieldWorldID)
		}
		if _q.withParent != nil {
			_spec.Node.AddColumnOnce(skill.FieldParentSkillID)
		}
	}
