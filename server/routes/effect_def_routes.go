...
func RegisterEffectDefRoutes(r *gin.Engine, client *db.Client) {
	effects := r.Group("/api/effects")
	effects.Use(middleware.AuthMiddleware())
	effects.Use(middleware.AdminMiddleware())
	{
		effects.GET("", listEffectDefs(client))
		effects.POST("", createEffectDef(client))
		effects.GET("/:id", getEffectDef(client))
		effects.PUT("/:id", updateEffectDef(client))
		effects.DELETE("/:id", deleteEffectDef(client))
	}
}
